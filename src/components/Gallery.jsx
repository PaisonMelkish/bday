 ...
 *
 * Drop into: src/components/Gallery.jsx
 * Requirements:
 *  - Tailwind CSS loaded (src/index.css)
 *  - Images in public/images/ (update IMAGES if needed)
 *
 * Behavior:
 *  - Thumbnails animate into view from random sides (left/right/top/bottom) using IntersectionObserver.
 *  - Masonry layout on wide screens; grid on small screens.
 *  - Lightbox with autoplay, swipe, keyboard, download preserved.
 */
...
export default function Gallery({ onNext }) {
  ...
  // refs for swipe handling + observer container
  const startXRef = useRef(null);
  const containerRef = useRef(null);
  const observerRef = useRef(null);

  // assign animation directions only once
  useEffect(() => {
    // choose random direction for each thumbnail and store as data attribute
    const thumbs = document.querySelectorAll("[data-thumb-index]");
    thumbs.forEach((el) => {
      if (el.dataset.dir) return; // already assigned
      const dirs = ["left", "right", "top", "bottom"];
      el.dataset.dir = dirs[Math.floor(Math.random() * dirs.length)];
      // initial hidden state class
      el.classList.add("thumb-hidden");
    });

    // intersection observer to reveal thumbs
  ...
  // injected CSS (entrance animations + masonry + small polish)
  const injectedCSS = `
    ...
    /* enter from left/right/top/bottom */
  ...
  useEffect(() => {
    const handler = (e) => {
      ...
      if (e.key === "ArrowRight") { goNext(); setAutoplay(false); }
      if (e.key === "ArrowLeft") { goPrev(); setAutoplay(false); }
  ...
  // swipe handlers for touch / mouse on the lightbox container
  useEffect(() => {
    ...
    const onEnd = (e) => {
      ...
      if (Math.abs(diff) > 40) {
        if (diff < 0) { goNext(); setAutoplay(false); } else { goPrev(); setAutoplay(false); }
      }
  ...
  useEffect(() => {
    if (lightboxOpen && autoplay) startAutoplay(3000);
    else stopAutoplay();
    return () => stopAutoplay();
  }, [autoplay, lightboxOpen]);

  // preload neighbors when index changes (improves UX)
  useEffect(() => {
    if (!lightboxOpen) return;
    const next = (index + 1) % IMAGES.length;
    const prev = (index - 1 + IMAGES.length) % IMAGES.length;
    [IMAGES[next].src, IMAGES[prev].src].forEach((s) => { const i = new Image(); i.src = s; });
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [index, lightboxOpen]);
    ...
    <>
      ...
      <div className="py-8 px-4 w-full max-w-6xl mx-auto">
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex items-center gap-3">
            <button onClick={() => window.scrollTo({ top: 0, behavior: "smooth" })} className="px-3 py-2 rounded-lg bg-white border shadow-sm text-sm">Scroll Top</button>
            <button onClick={() => typeof onNext === "function" && onNext()} className="px-3 py-2 rounded-lg bg-rose-500 text-white text-sm font-medium shadow hover:brightness-95">Surprise →</button>
          </div>
        </div>
          ...
          {IMAGES.map((img, i) => (
            ...
            >
              <img src={img.src} alt={img.caption || `Photo ${i + 1}`} loading="lazy"
                   className="w-full h-40 object-cover group-hover:scale-105 transition-transform duration-300" draggable={false} />
              <div className="absolute left-2 bottom-2 px-2 py-1 rounded bg-black/40 text-white text-xs">{img.caption}</div>
            </button>
          ))}
        </div>

        {/* masonry layout on larger screens */}
          ...
          <div className="masonry">
            {IMAGES.map((img, i) => (
              <div key={img.src} className="masonry-item">
                ...
                >
                  <img src={img.src} alt={img.caption || `Photo ${i + 1}`} loading="lazy"
                       className="w-full object-cover rounded-xl" style={{ display: "block", width: "100%" }} draggable={false} />
                </button>
              </div>
            ))}
          </div>
      ...
      {lightboxOpen && (
        <div className="fixed inset-0 z-[1200] flex items-center justify-center bg-black/85">
          {/* close */}
          <button onClick={closeLightbox} className="absolute top-6 right-6 z-[1250] bg-white/90 rounded-full p-2 shadow">✕</button>

          {/* autoplay / download controls */}
          <div className="absolute top-6 left-6 z-[1250] flex items-center gap-2">
            <button onClick={() => setAutoplay((s) => !s)} className="px-3 py-2 bg-white/90 rounded shadow text-sm" aria-pressed={autoplay}>
              {autoplay ? "Pause" : "Play"}
            </button>
            <button onClick={downloadCurrent} className="px-3 py-2 bg-white/90 rounded shadow text-sm">Download</button>
          </div>
          ...
          <div ref={containerRef} className="relative max-w-6xl w-full mx-4 md:mx-8">
            <div className="relative rounded-xl overflow-hidden bg-white/5">
              {IMAGES.map((img, i) => {
                ...
                return (
                  <img key={img.src}
                       ...
                       className={`absolute inset-0 w-full h-[60vh] md:h-[72vh] object-contain mx-auto left-0 right-0 transition-opacity duration-500 ${visible ? "opacity-100 lightbox-fade" : "opacity-0 pointer-events-none"}`}
              ...
              <div className="absolute left-0 right-0 bottom-0 px-6 py-4 bg-gradient-to-t from-black/60 to-transparent flex items-center justify-between">
                ...
                <div className="flex items-center gap-3">
                  <button onClick={() => { goPrev(); setAutoplay(false); }} className="px-3 py-2 bg-white/80 rounded shadow text-sm">‹</button>
                  <button onClick={() => { goNext(); setAutoplay(false); }} className="px-3 py-2 bg-white/80 rounded shadow text-sm">›</button>
                </div>
              </div>
            </div>
            ...
            <div className="mt-4 flex items-center gap-3 overflow-x-auto py-2">
              {IMAGES.map((img, i) => (
                <button key={img.src + "-thumb"} onClick={() => { setIndex(i); setAutoplay(false); }} className={`rounded-lg overflow-hidden border ${i === index ? "ring-2 ring-rose-300" : ""}`}>
